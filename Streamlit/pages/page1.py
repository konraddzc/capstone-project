...
# Define project-relative paths used for locating data, models, and logs
PROJECT_ROOT = Path(__file__).resolve().parents[2]
DATA_PROCESSED = PROJECT_ROOT / "data" / "processed"
MODELS_DIR = PROJECT_ROOT / "notebooks"
LOG_FILE = PROJECT_ROOT / "src" / "logs" / "etl_pipeline.log"
...
# Compute dataset availability and number of rows by counting CSV lines
...
# Check whether each model file exists in the expected directory
def get_model_stats():
    files = {
...
# Retrieve last ETL pipeline timestamp using the log file modification time
...
# Collect system-level statistics to display in the dashboard
ds_stats = get_dataset_stats()
model_stats = get_model_stats()
last_etl = get_last_etl_run()

# Layout: three columns for system status, dataset overview, and model overview
top_left, top_mid, top_right = st.columns([1.2, 1.1, 1.1])

# LEFT PANEL — System-level availability indicators
with top_left:
    st.markdown(
        """
    ...
    # Display last ETL execution time or fallback message
    st.markdown(
...
# MIDDLE PANEL — Dataset row counts and availability
with top_mid:
    st.markdown(
        """
    ...
    # List each dataset and its row count or missing state
...
# RIGHT PANEL — Model availability badges
with top_right:
    st.markdown(
        """
    ...
    # Print badge for each model file
...
# SECOND ROW — Pipeline diagram and usage instructions
...
with mid_left:
    ...
    # Build Graphviz diagram of the ETL and training pipeline
    ...
    # Render diagram inside Streamlit
...
with mid_right:
    ...
    # Usage instructions for interacting with the dashboard
    st.markdown(
        """
